...
#pragma endregion sboxs

#define S(x,i)(SBoxes[i][x.w.byte##i])
#define bf_F(x)(((S(x,0) + S(x,1)) ^ S(x,2)) + S(x,3))
#define ROUND(a,b,n)(a.dword ^= bf_F(b) ^ PArray[n])

YCBFEncryption::YCBFEncryption(const char *pwd) 
{ 
...
YCBFEncryption::~YCBFEncryption(void)
{
...
//
bool YCBFEncryption::encode(const char *plain, unsigned int inLen, char * cipher, unsigned int& outLen)
{
	if (plain != cipher)
		memcpy(cipher, plain, inLen);
	
	outLen = getOutputLength(inLen);
	for(DWORD lCount = 0; lCount < outLen; lCount += 8)
	{
		// if padding is required.
		if (lCount + 8 > inLen)
		{
			// point at byte past the end of actual data
			char *po = (char*)cipher + (inLen - lCount);
			// padding with 255 (FF) characters.
			for(DWORD i = 0; i < (outLen - inLen); i++)
				*po++ = (char)0;
		}
		Enc_encipher((DWORD*)cipher, (DWORD*)(cipher + 4));
		cipher += 8;
	}
	return (outLen > 0);
}
...
//
bool YCBFEncryption::decode(const char *cipher, unsigned int inLen, char * plain, unsigned int& outLen)
{
	if (plain != cipher)
		memcpy(plain, cipher, inLen);

	for (DWORD lCount = 0; lCount < inLen; lCount += 8)
	{
	 	Enc_decipher((DWORD*)plain, (DWORD*)(plain + 4));
		plain += 8;
	}
...
void YCBFEncryption::Enc_encipher(DWORD *xl, DWORD *xr)
{
	union aword  Xl, Xr;
...
void YCBFEncryption::Enc_decipher(DWORD *xl, DWORD *xr)
{
   union aword  Xl;
   union aword  Xr;
...
DWORD YCBFEncryption::getOutputLength(DWORD lInputLong)
{
	DWORD 	lVal;
